...
| `inpatient_records` | 每日住院记录，保存病情、诊疗方案摘要和治疗费用。 |
| `inpatient_record_items` | 住院记录明细，保存药品或治疗项目明细。 |
...
| `bills` | 费用单，可关联门诊或住院。 |
| `bill_items` | 费用明细。 |
| `payments` | 支付记录，记录每笔缴费。 |
| `prepaid_records` | 预缴记录，记录住院预缴/扣费/退款流水。 |
...
erDiagram
    ...
    INPATIENT_RECORDS ||--o{ INPATIENT_RECORD_ITEMS : contains
    MEDICINES ||--o{ INPATIENT_RECORD_ITEMS : used_by
    PATIENTS ||--o{ BILLS : charged
    BILLS ||--o{ BILL_ITEMS : contains
    BILLS ||--o{ PAYMENTS : paid_by
    ADMISSIONS ||--o{ PREPAID_RECORDS : has
```
...
- 医生和病人都可以绑定一个登录账号。
- `users.permissions` 存储 JSON 权限标识数组，控制角色可访问的功能和数据。
...
- 一条住院档案只属于一位病人。
- 每次住院安排一个科室、一位主治医生，病床可在入院时分配或后续分配。
- 一个病床可以有多次历史住院记录，但同一时间只能被一个未出院病人占用。
- 住院档案记录预缴余额，通过 `prepaid_records` 记录每次充值、扣费和退款流水。
- 每次住院每天至少可生成一条住院记录，记录病情描述、诊疗方案摘要和治疗费用。
- 一条住院记录可以包含多条明细，明细可以是药品，也可以是非药品治疗项目。
...
- 支付记录单独存 `payments` 表，支持资金流水追溯。
- 住院预缴/扣费/退款通过 `prepaid_records` 表记录，支持余额追溯。
...
### 7.16 `inpatient_record_items` 住院记录明细表
...
| `inpatient_record_items` | `(inpatient_record_id)` | 住院记录明细查询 |
| `bills` | `(patient_id, created_at)` | 病人费用查询 |
| `payments` | `(bill_id)` | 账单支付查询 |
| `payments` | `(patient_id, paid_at)` | 病人支付历史 |
| `prepaid_records` | `(admission_id)` | 住院预缴流水 |
| `prepaid_records` | `(patient_id, created_at)` | 病人预缴历史 |
...
- 住院：通过 `admissions -> inpatient_records -> inpatient_record_items` 查询。
...
| 病人缴费 | `bills`, `bill_items`, `payments` |
| 住院预缴与扣费 | `prepaid_records`, `admissions` |
| 住院建档 | `admissions`, `wards`, `beds` |
| 每日住院记录 | `inpatient_records`, `inpatient_record_items` |
| 查询治疗记录 | 门诊表、住院表、费用表 |
...
6. 账单支付成功后应将账单状态改为 `PAID`，并插入支付记录。
7. 住院预缴余额不足时应自动将住院档案状态改为 `SUSPENDED`。
8. 预缴/扣费/退款全部通过 `prepaid_records` 记录，每次变动保存余额快照。
9. 出院结算时多退少补，预缴余额结余生成退款记录，不足生成补缴账单。
